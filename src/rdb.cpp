...
namespace train_set {
    ...
    bool RDBManager::dump(KVStorage *storage, std::string &err) {
        ...
        for (auto &item: strs) {
            ...
            if (write(fd, res.c_str(), res.size()) < 0) {
                close(fd);
                err = "write string";
                return false;
            }
        ...
        for (auto &item: hash) {
            ...
            for (auto &field_val: r.field_vals) {
                ...
                fv_line.append(std::to_string(field_val.first.size()).append(" ").
                        ...
                        append(std::to_string(field_val.second.size())).
                        append(" ").
                        append(field_val.second).append("\n")
                );
                if (write(fd, fv_line.c_str(), fv_line.size()) < 0) {
                    close(fd);
                    err = "write hash field_val";
                    return false;
                }
            }
        }
        ...
        for (auto &item: zset) {
            ...
            for (auto &z: zset) {
                ...
                if (write(fd, z_line.c_str(), z_line.size()) < 0) {
                    close(fd);
                    err = "write zset item";
                    return false;
                }
    ...
    bool RDBManager::load(KVStorage *storage, std::string &err) {
        ...
        auto readline = [&](std::string &out) -> bool {
            ...
            out.assign(file_data.data() + file_data_pos, end - file_data_pos);
            file_data_pos = end + 1;
            return true;
        };
        std::string line;
        ...
        // STRING section
        if (!readline(line)) {
            err = "read STRING section failed";
            return false;
        }
        ...
        if (line.rfind("HASH", 0) != 0) {
            err = "invalid HASH section header";
            return false;
        }
        ...
        for (int i = 0; i < hash_count; i++) {
            ...
            std::string key = line.substr(line_pos, static_cast<size_t>(klen));
            line_pos += static_cast<size_t>(klen) + 1;
            ...
            for (int j = 0; j < nf; j++) {
                ...
                std::string field = line.substr(line_pos, static_cast<size_t>(flen));
                line_pos += static_cast<size_t>(flen) + 1;

                std::string vlen_s;
                nextTok(line, line_pos, vlen_s);
                int vlen = std::stoi(vlen_s);
                std::string val = line.substr(line_pos, static_cast<size_t>(vlen));
        ...
        for (int i = 0; i < zset_count; i++) {
            ...
            for (int j = 0; j < nmems; j++) {
                ...
                std::string member = line.substr(line_pos, static_cast<size_t>(mlen));
